...
public protocol DebitNoteXMLTransforming: Sendable {
    ...
    func transform(_ note: NotaDebito, signature: SignatureInformation) throws -> String
}
...
public struct DebitNoteXMLTransformer: DebitNoteXMLTransforming, Sendable {
    ...
    public func transform(_ note: NotaDebito, signature: SignatureInformation) throws -> String {
        try transform(note, signature: Optional(signature))
    }

    private func transform(_ note: NotaDebito, signature: SignatureInformation?) throws -> String {
    ...
    private func write(_ line: DebitNoteLine, to writer: inout XMLWriter) {
        ...
        if let quantity = line.quantity {
            writer.element(
                "cbc:DebitedQuantity",
                text: formatDecimal(quantity.value),
                attributes: [
        ...
        if let price = line.price {
            ...
            write("cbc:PriceAmount", amount: price, to: &writer)
